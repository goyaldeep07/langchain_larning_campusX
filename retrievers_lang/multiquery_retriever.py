...
vector_store = FAISS.from_documents(documents=all_docs, embedding=embed_model)
...
multiquery_retriever = MultiQueryRetriever.from_llm(
    llm=ChatOpenAI(), retriever=vector_store.as_retriever(search_kwargs={"k": 5})
)
...
for ind, doc in enumerate(similarity_result):
    print(ind + 1, doc.page_content)

print()
for ind, doc in enumerate(multiquery_result):
    print(ind + 1, doc.page_content)
